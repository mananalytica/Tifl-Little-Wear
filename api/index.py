...
@app.get("/api/sitemap-dynamic.xml")
def sitemap_dynamic():
    """Auto-generated sitemap covering pages that don't exist as static
    files: individual product pages (product.html?id=...) and individual
    tailor profile pages (master-tailor.html?slug=...). Referenced from the
    static /sitemap.xml index alongside sitemap-pages.xml, so new products
    or tailors show up for Google without a manual sitemap edit."""
    conn = get_conn()
    products = feed_rows(conn)
    tailor_rows = conn.execute(
        "SELECT slug, created_at FROM tailors WHERE active = true ORDER BY created_at"
    ).fetchall()
    conn.close()

    urls = []
    for p in products:
        loc = f"{SITE_URL}/product.html?id={p['product_id']}"
        lastmod = str(p.get("created_at") or "")[:10]
        urls.append((loc, lastmod, "0.6"))
    for slug, created_at in tailor_rows:
        loc = f"{SITE_URL}/master-tailor.html?slug={slug}"
        lastmod = str(created_at or "")[:10]
        urls.append((loc, lastmod, "0.5"))
  ...
  </url>""" for loc, lastmod, priority in urls
    )
