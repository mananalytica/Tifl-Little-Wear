...
------------------------------------------------------------------------ */
function responsiveWidth(pct, maxPx){
  const viewport = Math.min(window.innerWidth || 900, 1180);
  const dpr = Math.min(window.devicePixelRatio || 1, 2);
  return Math.round(Math.min(viewport * (pct/100) * dpr, maxPx || 1600));
}
...
async function initHeroGallery(){
  ...
  function loadSlideMedia(i){
    ...
    media.innerHTML = productThumbHTML(items[i], '46%', {width:responsiveWidth(46, 900), eager: i===0});
    const img = media.querySelector('img');
...
async function initTailorPage(){
  ...
  document.getElementById('tpPhoto').innerHTML = tailorPhotoHTML(t, {width:responsiveWidth(50, 700), eager:true});
...
async function initProductPage(){
  ...
  // Fill in the page
  document.getElementById('pdMedia').innerHTML = productThumbHTML(p, '55%', {width:responsiveWidth(55, 900), eager:true});
  ...
  if(thumbsRoot){
    ...
    thumbsRoot.querySelectorAll('.pd-thumb').forEach(t=>{
      t.addEventListener('click', ()=>{
        ...
        document.getElementById('pdMedia').innerHTML = productThumbHTML(Object.assign({}, p, {image_url: t.dataset.img}), '55%', {width:responsiveWidth(55, 900), eager:true});
      });
    });
  }
...
/* ============================================================
   ADMIN PAGE (admin.html) logic has moved to admin.js — see that
   file's header comment for why. Loaded only on admin.html, right
   after this file.
============================================================= */
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  initContactPage();
  initProductPage();
  // Defined in admin.js, which is only loaded on admin.html — guarded so
  // every other page's dispatcher doesn't throw a ReferenceError here and
  // silently skip initLiveSellPage/initAccountPage/initCheckoutPage below.
